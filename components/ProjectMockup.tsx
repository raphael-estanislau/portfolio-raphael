...
/* ------------------------------------------------------------------ */
/*  Inline chart helpers · dark "engineer" palette                     */
/* ------------------------------------------------------------------ */
  ...
  return (
    <svg viewBox="0 0 220 64" className="h-16 w-full" preserveAspectRatio="none">
      {values.map((v, i) => {
        ...
        return (
          <rect key={i} x={x} y={60 - h} width={bw} height={h} rx={1.5} fill={last ? ACCENT : BAR} />
        );
      })}
    </svg>
  );
...
function Spark({ values, fill = false }: { values: number[]; fill?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 220 60" className="h-14 w-full" preserveAspectRatio="none">
      {fill && <path d={`${d} L220,60 L0,60 Z`} fill={ACCENT} opacity={0.1} />}
...
function Frame({
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-panel shadow-[0_24px_50px_-28px_rgba(0,0,0,0.7)]">
      <div className="flex items-center justify-between border-b border-line bg-navy/60 px-4 py-2.5">
        ...
        {tabs.map((tb, i) => (
          <span
            ...
            className={`pb-2 font-mono text-[11px] ${
              i === active ? "border-b-2 border-accent text-accent" : "text-muted"
            }`}
          >
            {tb}
          </span>
...
function Kpi({ label, value, delta }: { label: string; value: string; delta?: string }) {
  return (
    <div className="rounded-lg border border-line bg-panel2/40 px-3 py-2.5">
      <p className="font-mono text-[9.5px] uppercase tracking-widest text-muted">{label}</p>
      <p className="tnum mt-1 text-lg font-semibold leading-none text-lightest">{value}</p>
      {delta && <p className="tnum mt-1 font-mono text-[10px] text-accent">{delta}</p>}
    </div>
  );
}
          ...
          <ul className="mt-2 space-y-1.5">
            {[
              ...
              <li key={n} className="flex items-center justify-between text-[11px]">
                ...
                <span className="tnum text-light">{v}</span>
              </li>
            ))}
          </ul>
        </div>
    ...
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[auto_1fr]">
        <div className="flex w-full flex-col items-center justify-center rounded-lg border border-line bg-panel2/40 p-3 sm:w-32">
          ...
          <p className="tnum -mt-12 text-2xl font-semibold text-lightest">{score}</p>
          <p className="mt-9 text-center font-mono text-[9px] uppercase tracking-widest text-muted">
        ...
        <div className="rounded-lg border border-line bg-panel2/40 p-3">
          ...
          <ul className="mt-2 space-y-1.5">
            {[
              ...
              <li key={n as string} className="flex items-center justify-between text-[11px]">
                <span className={hl ? "font-medium text-accent" : "text-slate"}>{n}</span>
                <span className="tnum text-light">{v}</span>
              </li>
            ))}
          </ul>
...
function VmMock() {
  ...
  return (
    ...
    >
        ...
        <div className="relative h-[148px] overflow-hidden rounded-lg border border-line bg-navy/60">
          ...
          {pins.map(([x, y, c], i) => (
            <span
              ...
              style={{ left: `${x}%`, top: `${y}%` }}
            >
              <span className="block h-3 w-3 rounded-full ring-2 ring-panel" style={{ background: c }} />
            </span>
          ))}
          <span className="absolute bottom-2 left-3 font-mono text-[9px] uppercase tracking-widest text-muted">
      ...
      <div className="mt-3 flex items-center gap-4 rounded-lg border border-line bg-panel2/40 px-3 py-2">
        {[
          [L("Alta venda", "High sales"), GREEN],
          [L("Atenção", "Watch"), AMBER],
          [L("Sem giro", "No rotation"), RED],
        ].map(([t, c]) => (
          <span key={t} className="flex items-center gap-1.5 text-[10px] text-slate">
